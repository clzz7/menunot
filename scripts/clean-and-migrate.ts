...
async function cleanAndMigrate() {
  console.log('🧹 Limpando e migrando dados...');
  
  try {
    const databaseUrl = process.env.DATABASE_URL || "postgresql://postgres.vzrpfxccwqfbeenhgkra:FhG1xqTecHYz7abU@aws-0-us-east-2.pooler.supabase.com:6543/postgres";
    ...
    await postgresDb.execute(sql`TRUNCATE TABLE order_items CASCADE`);
    await postgresDb.execute(sql`TRUNCATE TABLE orders CASCADE`);
    await postgresDb.execute(sql`TRUNCATE TABLE coupons CASCADE`);
    await postgresDb.execute(sql`TRUNCATE TABLE products CASCADE`);
    await postgresDb.execute(sql`TRUNCATE TABLE categories CASCADE`);
    await postgresDb.execute(sql`TRUNCATE TABLE customers CASCADE`);
    await postgresDb.execute(sql`TRUNCATE TABLE establishments CASCADE`);
    ...
    const transformOrderItems = orderItems.map(item => ({
      ...
      total_price: item.total_price || (item.product_price || 0) * (item.quantity || 1)
    }));
    ...
    console.log('📱 Aplicação agora está usando PostgreSQL no Supabase');
